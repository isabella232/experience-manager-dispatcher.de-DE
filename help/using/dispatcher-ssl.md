...
description: Erfahren Sie, wie Dispatcher für die Kommunikation mit AEM mithilfe von SSL-Verbindungen konfiguriert wird.
seo-description: Erfahren Sie, wie Dispatcher für die Kommunikation mit AEM mithilfe von SSL-Verbindungen konfiguriert wird.
...
>[!NOTE]
>
>Vorgänge im Zusammenhang mit SSL-Zertifikaten sind von Drittanbieterprodukten abhängig. Sie werden nicht vom Adobe Platinum Maintenance- und Support-Vertrag abgedeckt.
...
* AEM 6.2: [HTTP über SSL aktivieren](https://helpx.adobe.com/experience-manager/6-2/sites/deploying/using/config-ssl.html)
* AEM 6.1: [HTTP über SSL aktivieren](https://docs.adobe.com/content/docs/en/aem/6-1/deploy/configuring/config-ssl.html)
* Ältere AEM-Versionen: [](https://helpx.adobe.com/experience-manager/aem-previous-versions.html)auf dieser Seite.
...
Eine Anforderung über Apache -2.2 mit `mod_ssl` Kopfzeilen, die dem folgenden Beispiel ähneln:
...
* Das `renders` Element enthält eine Eigenschaft mit dem Namen `secure` value `1`.
...
1. [Installieren](dispatcher-install.md) Sie die neueste Version des Dispatchers für Ihre Plattform. Verwenden Sie eine Dispatcher-Binärdatei, die SSL unterstützt (SSL befindet sich im Dateinamen, z. B. dispatcher-apache2.4-linux-x86-64-ssl10-4.1.7.tar).
...
Verwenden Sie das Render-Zertifikat mit den Anweisungen unter &quot;SSL *aktivieren im Abschnitt&quot; Veröffentlichungsinstanz* &quot; , um den HTTP-Dienst der Renderinstanz für die Verwendung von SSL zu konfigurieren:

* AEM 6.2: [HTTP über SSL aktivieren](https://helpx.adobe.com/experience-manager/6-2/sites/deploying/using/config-ssl.html)
* AEM 6.1: [HTTP über SSL aktivieren](https://docs.adobe.com/content/docs/en/aem/6-1/deploy/configuring/config-ssl.html)
* Ältere AEM-Versionen: [auf dieser Seite.](https://helpx.adobe.com/experience-manager/aem-previous-versions.html)
...
1. Verwenden Sie einen Texteditor oder den `cat` Befehl, um den unverschlüsselten privaten Schlüssel und das Zertifikat in einer einzelnen Datei zu kombinieren, die dem folgenden Beispiel ähnelt:
...
Fügen Sie die folgenden Eigenschaften der Konfiguration des [Dispatcher-Moduls hinzu](dispatcher-install.md#main-pars-55-35-1022) (in `httpd.conf`):
...
* `DispatcherCheckPeerCN`: Ob die Überprüfung `On`der Hostnamen-Prüfung `Off`für Remote-Server-Zertifikate aktiviert () oder deaktiviert () ist.
