...
description: Erfahren Sie, wie Sie mit Dispatcher Seitenanforderungen in mehreren Webdomänen verarbeiten.
seo-description: Erfahren Sie, wie Sie mit Dispatcher Seitenanforderungen in mehreren Webdomänen verarbeiten.
...
discoiquuid: 40 d 91 d 66-c 99 b -422 d -8 e 61-c 0 23272 ef
...
Seiten für `BrandA.com` werden unten `/content/sitea`gespeichert. Client-Anforderungen nach der URL `https://BrandA.com/en.html` werden die wiedergegebene Seite für den `/content/sitea/en` Knoten zurückgegeben. Auf ähnliche Weise werden Seiten unten `BrandB.com` gespeichert `/content/siteb`.
...
* (Empfohlen) Die AEM-Veröffentlichungsinstanz verwendet die Sling-Zuordnung für die Ressourcenauflösung zur Implementierung von Regeln zum Neuschreiben interner URLs. Domänen-URLs werden in Inhaltsrepositorypfade übersetzt. (Siehe [AEM schreibt eingehende URLs neu](dispatcher-domains.md#main-pars-title-2).)
* Der Webserver verwendet Regeln zum Neuschreiben interner URLs, die Domänen-URLs in Cachepfade übersetzen. (Siehe [Webserver Reschreibt eingehende urls](dispatcher-domains.md#main-pars-title-1).)
...
* Der Dispatcher Flush-Replikationsagent wird auf der AEM-Veröffentlichungsinstanz konfiguriert. (Siehe [Ungültiges Dispatcher-Cache aus einer Veröffentlichungsinstanz ungültig](page-invalidate.md)machen.)
...
* Das AEM-Inhalts-Repository und der Dokumentenstamm des Webservers verwenden die folgenden Dateistrukturen (der Dokumentenstamm des Apache-Webservers lautet/`usr/lib/apache/httpd-2.4.3/htdocs)`:
...
Dispatcher-Anforderungen für das Rendern von Seiten identifizieren die Seite mithilfe der URL, die vom Webserver übergeben wurde. Wenn die URL einen Domänennamen enthält, lösen Sling-Zuordnungen die URL zum Inhalt auf. Die folgende Grafik zeigt eine Zuordnung der `branda.com/en.html` URL zu `/content/sitea/en` dem Knoten.

![](assets/chlimage_1-10.png)

Der Dispatcher-Cache spiegelt die Repositoryknotenstruktur wider. Wenn daher Seitenaktivierungen eintreten, sind die resultierenden Anforderungen zum unvollständigen Aufrufen der zwischengespeicherten Seite keine URL- oder Pfadübersetzungen erforderlich.
...
* Konfigurieren Sie für jede Domäne den Dokumenstamm, damit dieser mit dem Ordner im Repository übereinstimmt, der den Webinhalt der Domäne enthält.
...
Mit dieser Konfiguration führt der Webserver die folgenden Aktionen durch, wenn eine Anforderung für `https://branda.com/en/products.html`Folgendes bereitgestellt wird:
...
* Die `/virtualhosts` Eigenschaft wird auf den Domänennamen festgelegt. Diese Eigenschaft ermöglicht dem Dispatcher die Zuordnung der Farm zur Domäne.
* Die `/filter` Eigenschaft erlaubt den Zugriff auf den Pfad der Anforderungs-URL, die nach dem Domänennamen-Teil abgeschnitten wurde. Beispielsweise wird der Pfad für `https://branda.com/en.html` die URL interpretiert `/en.html`, sodass der Filter den Zugriff auf diesen Pfad zulassen muss.

* Die `/docroot` Eigenschaft wird im Dispatcher-Cache auf den Pfad des Stammordners der Domäne der Domäne gesetzt. Dieser Pfad wird als Präfix für die verkettete URL von der ursprünglichen Anforderung verwendet. Beispielsweise wird durch den Docroot-Prozess `/usr/lib/apache/httpd-2.4.3/htdocs/sitea` die Anforderung `https://branda.com/en.html` an die `/usr/lib/apache/httpd-2.4.3/htdocs/sitea/en.html` Datei aufgelöst.
...
* Die `/docroot` Eigenschaft wird auf den Standardandott des Webservers festgelegt. In der Regel ist dies der Ordner, in dem der `/content` Ordner erstellt wird. Ein Beispielwert für Apache unter Linux `/usr/lib/apache/httpd-2.4.3/htdocs`ist.
* Die `/filter` Eigenschaft erlaubt den Zugriff auf Dateien unter dem `/content` Ordner.

Die `/statfileslevel`Eigenschaft muss hoch genug sein, damit. csr-Dateien im Stammordner jedes virtuellen Hosts erstellt werden. Diese Eigenschaft ermöglicht es, den Cache für jede Domäne einzeln ungültig zu machen. Für das Beispiel-Setup wird der `/statfileslevel` Wert von `2` . csr-Dateien im `*docroot*/content/sitea` Ordner und im `*docroot*/content/siteb` Ordner erstellt.
...
Weitere Informationen zur Sling-Ressourcenzuordnung finden Sie unter [Zuordnungen für Ressourcenauflösungen](https://sling.apache.org/site/mappings-for-resource-resolution.html) in der Sling-Dokumentation.
...
* Stamm-Node der Inhaltsseite (unten `/content`)
* Der Design-Knoten, den die Seiten verwenden (unten `/etc/designs`)
* Der `/libs` Ordner

Nachdem Sie die Zuordnung für die Inhaltsseite erstellt haben, verwenden Sie zum Erkennen zusätzlicher erforderlicher Zuordnungen einen Webbrowser, um eine Seite auf dem Webserver zu öffnen. Suchen Sie in der Datei „error.log“ der Veröffentlichungsinstanz Meldungen zu Ressourcen, die nicht gefunden wurden. Die folgende Beispielmeldung zeigt an, dass eine Zuordnung `/etc/clientlibs` erforderlich ist:
...
In der folgenden Tabelle sind die Knoten aufgeführt, die die Ressourcenzuordnung für die Domäne „branda.com“ implementieren. Ähnliche Knoten werden für die `brandb.com` Domäne erstellt, z. `/etc/map/http/brandb.com`B. In allen Fällen sind Zuordnungen erforderlich, wenn Verweise im HTML-Code der Seite nicht ordnungsgemäß im Sling-Kontext aufgelöst werden.
...
Der Dispatcher Flush-Relikationsagent auf der AEM-Veröffentlichungsinstanz muss Invalidierungsanforderungen an die richtige Dispatcher-Farm senden. Verwenden Sie zum Bestimmen einer Farm die URI-Eigenschaft des Dispatcher Flush-Replikationsagenten (auf der Registerkarte „Transport“). Geben Sie den Wert der `/virtualhost` Eigenschaft für die Dispatcher-Farm ein, die für die Ungültigkeit des Cache konfiguriert ist:

`https://*webserver_name*:*port*/*virtual_host*/dispatcher/invalidate.cache`

Wenn Sie beispielsweise die `farm_flush` Farm des vorherigen Beispiels verwenden, lautet `https://localhost:80/invalidation_only/dispatcher/invalidate.cache`der URI.

![](assets/chlimage_1-12.png)

## Der Webserver schreibt eingehende URLs neu {#the-web-server-rewrites-incoming-urls}

Verwenden Sie die Funktion Ihres Webservers zum Neuschreiben interner URLs, um die domänenbasierten URLs in Dateipfade im Dispatcher-Cache zu übersetzen. Clientanfragen auf der `https://brandA.com/en.html` Seite werden beispielsweise in die `content/sitea/en.html`Datei im Dokumentenstamm des Webservers übersetzt.

![](assets/chlimage_1-13.png)

Der Dispatcher-Cache spiegelt die Repositoryknotenstruktur wider. Wenn daher Seitenaktivierungen eintreten, erfordern die resultierenden Anforderungen zur Ungültigkeit der zwischengespeicherten Seite keine URL oder Pfadübersetzungen.
...
* Jede virtuelle Domäne muss außerdem Dispatcher-bezogene Konfigurationen umfassen, wie auf der Seite [Installieren des Dispatchers](dispatcher-install.md) beschrieben.
* Das Dispatcher-Modul muss zur Verwendung der URL konfiguriert werden, die der Webserver neu geschrieben hat. (Siehe `DispatcherUseProcessedURL` die Eigenschaft unter [Installieren von Dispatcher](dispatcher-install.md).)
...
* Die `DispatherUseProcessedURL` Eigenschaft ist auf `1`. (Zeile 10)

Beispielsweise führt der Webserver bei Erhalt einer Anforderung mit der `https://brandA.com/en/products.html` URL die folgenden Aktionen durch:

* Ordnet die URL dem virtuellen Host mit `ServerName` als `brandA.com.` zu.
* Schreibt die URL in `/content/sitea/en/products.html.`
* Leitet die URL an den Dispatcher weiter.
...
Wenn der Webserver urls neu schreibt, erfordert Dispatcher eine einzelne Farm, die gemäß der [Konfigurieren des Dispatchers definiert](dispatcher-configuration.md)ist. Die folgenden Konfigurationen sind erforderlich, um die virtuellen Hosts und URL-Umbenennungsregeln des Webservers zu unterstützen:

* Die `/virtualhosts` Eigenschaft muss die servername-Werte für alle virtualhost-Definitionen enthalten.
* Die `/statfileslevel` Eigenschaft muss hoch genug sein, um. csr-Dateien in den Ordnern zu erstellen, die die Inhaltsdateien für jede Domäne enthalten.

Die folgende Beispielkonfigurationsdatei basiert auf der `dispatcher.any` Beispieldatei, die mit Dispatcher installiert wird. Die folgenden Änderungen sind erforderlich, um die Webserverkonfigurationen der vorherigen `httpd.conf` Datei zu unterstützen:

* Die `/virtualhosts` Eigenschaft führt dazu, dass Dispatcher Anforderungen für die `brandA.com` und `brandB.com` Domänen verarbeitet. (Zeile 12)
* Die `/statfileslevel` Eigenschaft wird auf 2 gesetzt, sodass CSR-Dateien in jedem Ordner erstellt werden, der den Webinhalt der Domäne enthält (Zeile 41): `/statfileslevel "2"`

Wie üblich ist der Dokumentenstamm des Zwischenspeichers identisch mit dem Dokumentenstamm des Webservers (Zeile 40): `/usr/lib/apache/httpd-2.4.3/htdocs`
...
* Der Generator analysiert HTML-Dokumente und generiert SAX-Ereignisse, wenn er die folgenden Elemente erkennt: „a“, „img“, „area“, „form“, „base“, „link“, „script“ und „body“. Der Generator-Alias `htmlparser`ist.
* Die Pipeline enthält die folgenden Transformierer: `linkchecker``mobile``mobiledebug``contentsync`. Der `linkchecker` Transformer externalisiert Pfade zu referenzierten HTML- oder HTM-Dateien, um beschädigte Links zu verhindern.
* Das Serialisierungsprogramm schreibt die HTML-Ausgabe. Der Serialisierungsprogrammalias lautet „htmlwriter“.

Der `/libs/cq/config/rewriter/default` Knoten definiert die Pipeline.
...
1. Implementieren Sie die `org.apache.sling.rewriter.TransformerFactory` Benutzeroberfläche. Diese Klasse erstellt Instanzen Ihrer Transformatorklasse. Geben Sie Werte für die `transformer.type` Eigenschaft (den Transformer Alias) an und konfigurieren Sie die Klasse als osgi-Dienst-Komponente.
1. Implementieren Sie die `org.apache.sling.rewriter.Transformer` Benutzeroberfläche. Um die Arbeit zu minimieren, können Sie die `org.apache.cocoon.xml.sax.AbstractSAXPipe` Klasse erweitern. Überschreiben Sie die Methode „startElement“, um das Neuschreibungsverhalten anzupassen. Diese Methode wird für jedes SAX-Ereignis aufgerufen, das an den Transformator übergeben wird.
...
>[!TIP]
>Sie können die transformerfactory stattdessen so konfigurieren, dass der Transformer in jeden definierten Autor eingefügt wird. Folglich müssen Sie keine Pipeline konfigurieren:
>
>* Legen Sie die `pipeline.mode` Eigenschaft `global`auf.
...
* Die myrewritertransformerfactory-Klasse instanziiert myrewritertransformer-Objekte instanziiert. Die Eigenschaft pipeline. type setzt den transformer Alias auf mytransformer. Um den Alias in eine Pipeline aufzunehmen, nimmt der Pipelinekonfigurationsknoten diesen Alias in die Liste der Transformatoren auf.
